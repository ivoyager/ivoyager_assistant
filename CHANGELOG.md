...
* Orbit accuracy test: `tools/orbit_accuracy_test.py` compares sim positions of 8 moons plus Earth against embedded JPL Horizons reference vectors at two dates via `get_body_position`, asserting parent-centered angular separation within per-body tolerances. Catches gross orbit propagation errors (element misinterpretation, epoch conversion, reference basis conventions). Supports `--launch` (auto-detects the Godot executable matching the project's `config/features` major), `--bodies` subset filter, `--host`, `--port`.
